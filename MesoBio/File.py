...
MesoBio.File defines private classes used in MesoBio.SeqIO and MesoBio.SearchIO for
...
def _open_for_random_access(filename):
    ...
    This functionality is used by the MesoBio.SeqIO and MesoBio.SearchIO index
...
class UndoHandle:
    ...
    def __init__(self, handle):
        ...
        raise Exception(
            ...
            "moved to MesoBio/SearchIO/_legacy/ParserSupport.py "
            "(which is the only module in Biopython still using "
            "UndoHandle."
        )


# The rest of this file defines code used in MesoBio.SeqIO and MesoBio.SearchIO
# for indexing
...
class _IndexedSeqFileProxy:
    ...
    This is subclasses in both MesoBio.SeqIO for indexing as SeqRecord
    objects, and in MesoBio.SearchIO for indexing QueryResult objects.
...
class _IndexedSeqFileDict(collections.abc.Mapping):
    ...
    This code is used in both MesoBio.SeqIO for indexing as SeqRecord
    objects, and in MesoBio.SearchIO for indexing QueryResult objects.
    ...
    As used in MesoBio.SeqIO, by default the SeqRecord's id string is used
    as the dictionary key. In MesoBio.SearchIO, the query's id string is
...
class _SQLiteManySeqFilesDict(_IndexedSeqFileDict):
    ...
    def __init__(
        ...
        if sqlite3 is None:
            ...
            from MesoBio import MissingPythonDependencyError
